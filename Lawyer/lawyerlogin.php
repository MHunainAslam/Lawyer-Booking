          ...
          <div class="content-wrapper full-page-wrapper d-flex align-items-center auth login-bg">
            <div class="card col-lg-4 mx-auto">
              <div class="card-body px-5 py-5">
                <h3 class="card-title text-left mb-3">Lawyer Login</h3>
                ...
                <?php 
                    if(isset($_POST["btnlogin"]))
                    {
                      ...
                      $login = mysqli_query($con,"select * from lawyer where Email ='$name' AND Password='$password'");
                      $check = mysqli_num_rows($login);
                      $getid = mysqli_fetch_array($login);
                      if($check>0)
                      {
                        header("location:index.php"); 
                        $_SESSION["LawyerEmails"] = $getid[0]; 
                        $_SESSION["LE"] = $getid[2]; 
                        
                        $_SESSION["limage"] = $getid[6];                  
                      }
                      else
                      {
                        echo "Login Failed";
